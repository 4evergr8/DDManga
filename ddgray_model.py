...
class GreyModel(nn.Module):
    def __init__(
            self,
            encoder_name='convnext-l',
            decoder_name='MultiScaleColorDecoder',
            num_input_channels=2,  # 输入2通道(XDoG + Gray)
            input_size=(256, 256),
            nf=512,
            num_output_channels=1,  # 输出灰度图1通道
            last_norm='Weight',
            do_normalize=False,
            num_queries=256,
            num_scales=3,
        ...
        self.refine_net = nn.Sequential(
            custom_conv_layer(num_queries + num_input_channels, num_output_channels, ks=1, use_activ=False,
                              norm_type=NormType.Spectral)
        )

        self.do_normalize = do_normalize
        # 灰度不做ImageNet标准化
        # 但如果需要，可以定义自己的mean/std
